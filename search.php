...
if(strlen($row->stock_code) != 1){
	if(stripos($row->corp_name,$_GET['str']) !==false){
	$chk = true;
	$data .= "<tr>";
...
$data .= "<td><form method='post'><input type='hidden' name='corp_code' value='".$row->corp_code."'><input type='hidden' name='stock_code' value='".$row->stock_code."'><input class='button_l'type='submit' name='add' id='add' value='추가' style='width:50px;height:30px;margin-top:9px;'></form></td>";
	$data .="</tr>";
		}
	}
}
...
<body> 
<h1 align="center">검색 결과 : <?=$_GET['str']?></h1>
<form method="get" action="search.php">
<input type="text" name="str" id="str" value="<?=$_GET['str']?>" style="height:25px;" placeholder="회사명을 입력하세요."/> <input class="button_l" type="submit" value="검색"> <input class="button_l" type="button" value="리셋" onclick="location.href='index.php'">
</form>
<? if($chk){ ?>
